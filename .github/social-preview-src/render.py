#!/usr/bin/env python3
"""Render a deterministic Active Window Highlight social-preview variant."""
...
def main() -> int:
    ...
    if not background.is_file() or not icon.is_file():
        raise SystemExit("Social-preview background or project icon is missing.")

    replacements = {
        "{{TAGLINE}}": html.escape(str(variant["tagline"])),
        "{{SUPPORTING}}": html.escape(str(variant["supporting"])),
    }
    ...
    with tempfile.TemporaryDirectory(prefix="active-window-highlight-social-preview-") as temporary:
        ...
        subprocess.run([svg_renderer, "--output", str(overlay_png), str(overlay_svg)], check=True)
        subprocess.run(command + [str(background), "-resize", f"{WIDTH}x{HEIGHT}!", str(canvas)], check=True)
        subprocess.run(command + [str(icon), "-filter", "Lanczos", "-resize", "82x82", str(app_icon)], check=True)
        subprocess.run(
            command
            + [
                str(canvas),
