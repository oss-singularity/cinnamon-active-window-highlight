#!/usr/bin/env python3
"""Render or verify every configured Active Window Highlight preview."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--check", action="store_true", help="fail if a committed preview differs from a fresh render")
...
def render(render_script: Path, variant: str, output: Path) -> None:
    subprocess.run(
        [sys.executable, str(render_script), "--variant", variant, "--output", str(output)],
        check=True,
    )
...
def main() -> int:
    ...
    if not args.check:
        for name in sorted(variants):
            render(render_script, name, configured_output(repo_dir, variants[name]["output"]))
        return 0

    stale: list[Path] = []
    with tempfile.TemporaryDirectory(prefix="active-window-highlight-social-preview-check-") as temporary:
